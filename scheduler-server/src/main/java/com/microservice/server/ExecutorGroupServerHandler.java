...
public class ExecutorGroupServerHandler extends SimpleChannelInboundHandler<MessageProtocolPoJo.MessageProtocol> {
    ...
    public void channelActive(ChannelHandlerContext ctx) throws Exception {
        ...
        // 要刷新缓存数据
        registryDetailService.autoFlushRedis();
    }

    @Override
    public void channelInactive(ChannelHandlerContext ctx) throws Exception {
        Channel channel = ctx.channel();
        log.info(">>> channelInactive {} 离线了~", channel.remoteAddress());
    ...
    protected void channelRead0(ChannelHandlerContext ctx, MessageProtocolPoJo.MessageProtocol messageProtocol) throws Exception {
        ...
        if (Objects.nonNull(protocolType) && StringUtils.isNotBlank(protocolType)) {
            ...
            if (Objects.nonNull(leader)) {
                ...
                this.clusterBroadcast(json);
            }
        } else {
            String content = messageProtocol.getContent();

            if (!Objects.deepEquals(content, PingPongUtils.PONG) && properties.isEnableChatRoom()) {
                // 1. 为了保证心跳正常，必须转发
                CHANNEL_GROUP.forEach(ch -> {
                    String msg = channel == ch ?
                            String.format("owner: %s, message: %s", channel.remoteAddress(), content) :
                            String.format("other: %s, message: %s", channel.remoteAddress(), content);
    ...
    private void clusterBroadcast(String message) {
