...
public class RedisDefaultGroupServerHandler {
    ...
    /**
     * 集群成员主动拉起，任务扫描，然后指派指定任务的执行器客户端
     *
     * @param tasks
     */
    @CacheLock(prefix = "scanningSchedulerTasks", expire = 60)
    public void scanningSchedulerTasks(Set<SchedulerTask> tasks) {
        Set<SchedulerTask> cronTasks = tasks.parallelStream().filter(f -> StringUtils.isNotBlank(f.getSchedulerCron())).collect(Collectors.toSet());
        Set<SchedulerTask> givenTasks = tasks.parallelStream().filter(f -> StringUtils.isBlank(f.getSchedulerCron())).collect(Collectors.toSet());
        ...
        if (!CollectionUtils.isEmpty(givenTasks)) {
            this.givenTasksHandler(givenTasks);
        }
    }

    private void cronTasksHandler(Set<SchedulerTask> cronTasks) {
        if (CollectionUtils.isEmpty(cronTasks)) {
            return;
        }
        LocalDateTime now = LocalDateTime.now();
        ...
        cronTasks.parallelStream().forEach(f -> {
            try {
                CronExpression cronExpression = new CronExpression(f.getSchedulerCron());
                boolean satisfiedBy = cronExpression.isSatisfiedBy(now);
                // 表达式执行时间已到，需要找出执行器并执行
                if (satisfiedBy) {
                    // 找出一个执行器开始执行
                    String actuatorName = f.getActuatorName();
                    log.info(">>>>>>> 执行器名字: {}", actuatorName);

                    // TODO 找出执行器集群成员, 推送给集群调度执行
                }
    ...
    private void givenTasksHandler(Set<SchedulerTask> givenTasks) {
        if (CollectionUtils.isEmpty(givenTasks)) {
            return;
        }
        ...
        givenTasks.parallelStream().forEach(f -> {
            LocalDateTime schedulerGivenTime = f.getSchedulerGivenTime();
            long second = schedulerGivenTime.toEpochSecond(ZoneOffset.ofHours(8));
            long schedulerGivenDelay = f.getSchedulerGivenDelay();
